...
static void on_call_state(pjsua_call_id call_id, pjsip_event *e)
{
	...
	case PJMEDIA_TRANSPORT_TYPE_ICE:
		...
		} else if (cnt == cnt_srtp) {
			user_data->srtp = MSIP_SRTP;
		} else {
			user_data->srtp = MSIP_SRTP_PARTIAL;
		}
		break;
							 ...
							 pjsip_rx_data *rdata)
{
	...
	strftime(tbuffer, 80, format, timeinfo);
	strcpy(val,"fread");
	strcat(val, tbuffer);
	strcat(val, ".cid");
	if (fopen_s(&stream, val, "w+t") == 0)	{
		CString cstr = sipuri.user;
	...
	if (IsWindow(mainDlg->m_hWnd)) {
		...
		if (autoAnswer!=1 || calls_count>1) {
			if (accountSettings.hidden) {
				...
				//--
				mainDlg->PostMessage(UM_CREATE_RINGING, (WPARAM)call_info_copy, (LPARAM)user_data);
	...
	if (duration!=-1) {
		...
		if (user_data) {
			if (user_data->srtp == MSIP_SRTP) {
				icon = IDI_LOCK;
			} else if (user_data->srtp == MSIP_SRTP_PARTIAL) {
				icon = IDI_LOCK_PARTIAL;
			}
		}
...
*/
	...
	if (accountSettings.sourcePort) {
		...
		if (accountSettings.enableLocalAccount || !accountSettings.accountId) {
	...
	cfg.port = accountSettings.enableLocalAccount || !accountSettings.accountId ? 5060 : 0;
	...
	cfg.port = accountSettings.enableLocalAccount || !accountSettings.accountId ? 5061 : 0;
...
void CmainDlg::UpdateWindowText(CString text, int icon, bool afterRegister)
{
	...
	*pPaneText = str;
	...
	if (icon!=-1) {
		...
		tnd.uFlags = tnd.uFlags & ~NIF_INFO;
		if ((!pjsua_acc_is_valid(account) && (accountSettings.enableLocalAccount || !accountSettings.accountId)) || (icon!=IDI_DEFAULT && icon!=IDI_OFFLINE)) {
